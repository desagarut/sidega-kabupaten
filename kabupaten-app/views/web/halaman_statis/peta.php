...
(function()
{
  ...
  {
		<?php if (!empty($kabupaten['lat']) AND !empty($kabupaten['lng'])): ?>
			var posisi = [<?=$kabupaten['lat'].",".$kabupaten['lng']?>];
			var zoom = <?=$kabupaten['zoom'] ?: 10?>;
		<?php elseif (!empty($kabupaten['path'])): ?>
			var wilayah_desa = <?=$kabupaten['path']?>;
    ...
    <?php if (!empty($kabupaten['path'])): ?>
      mymap.fitBounds(<?=$kabupaten['path']?>);
    <?php endif; ?>
    ...
    <?php if (!empty($kabupaten['path'])): ?>
      set_marker_desa_content(marker_kabupaten, <?=json_encode($kabupaten)?>, "<?=ucwords($this->setting->sebutan_desa).' '.$kabupaten['nama_desa']?>", "<?= favico_kabupaten()?>", '#isi_popup');
    <?php endif; ?>

    //OVERLAY WILAYAH DUSUN
    <?php if (!empty($dusun_gis)): ?>
      set_marker_content(marker_dusun, '<?=addslashes(json_encode($dusun_gis))?>', '<?=ucwords($this->setting->sebutan_dusun)?>', 'dusun', '#isi_popup_dusun_', '<?= favico_kabupaten()?>');
    <?php endif; ?>

    //OVERLAY WILAYAH RW
    <?php if (!empty($rw_gis)): ?>
      set_marker_content(marker_rw, '<?=addslashes(json_encode($rw_gis))?>', 'RW', 'rw', '#isi_popup_rw_', '<?= favico_kabupaten()?>');
    <?php endif; ?>

    //OVERLAY WILAYAH RT
    <?php if (!empty($rt_gis)): ?>
      set_marker_content(marker_rt, '<?=addslashes(json_encode($rt_gis))?>', 'RT', 'rt', '#isi_popup_rt_', '<?= favico_kabupaten()?>');
    <?php endif; ?>
    ...
    mymap.on('overlayadd', function (eventLayer) {
      if (eventLayer.name === 'Peta Wilayah Desa') {
        setlegendPetaDesa(legenda_desa, mymap, <?=json_encode($kabupaten)?>, '<?=ucwords($this->setting->sebutan_desa)?>', '<?=$kabupaten['nama_desa']?>');
      }
      ...
      <div class="col-md-12">
        <div id="map">
          <div class="leaflet-top leaflet-left">
            <?php $this->load->view("gis/content_desa_web.php", array('kabupaten' => $kabupaten, 'list_ref' => $list_ref, 'wilayah' => ucwords($this->setting->sebutan_desa.' '.$kabupaten['nama_desa']))) ?>
