...
class Front extends Web_Controller {
	...
	public function index($p=1)
	{
		...
		$this->track_model->track_kabupaten('front');
...
*/
	...
	*/
	...
	public function peta()
	{
		...
		$data['kabupaten'] = $this->config_model->get_data();
		$data['title'] = 'Peta ' . ucwords($this->setting->sebutan_kabupaten. ' ' . $data['kabupaten']['nama_kabupaten']);
	...
	public function status_idm()
	{
		...
		$this->_get_common_data($data);
		$kode_desa = $data['kabupaten']['kode_desa'];
		if ($this->data_publik->has_internet_connection())
		{
			$this->data_publik->set_api_url("https://idm.kemendesa.go.id/open/api/kecamatan/rumusan/$kode_desa/2020", "idm_$kode_desa")
				->set_interval(7)
				->set_cache_folder(FCPATH.'kabupaten');

			$idm = $this->data_publik->get_url_content();
			if ($idm->body->error)
			{
				$idm->body->mapData->error_msg = $idm->body->message . " : " . $idm->header->url . "<br><br>" .
					"Periksa Kode Desa di Identitas Desa. Masukkan kode lengkap, contoh '3205282001'<br>";
			}
