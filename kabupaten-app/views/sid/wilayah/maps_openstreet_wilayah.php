...
<div class="content-wrapper">
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="<?= site_url('hom_sid')?>"><i class="fa fa-home"></i> Beranda</a></li>
          ...
          <div class="box-body">
            ...
            <div class='box-footer'>
              <div class='col-xs-12'> <a href="<?= $tautan['link'] ?>" class="btn btn-social btn-box bg-purple btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Kembali"><i class="fa fa-arrow-circle-o-left"></i> Kembali</a> <a href="#" class="btn btn-social btn-box btn-success btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" download="SIDeGa.gpx" id="exportGPX"><i class='fa fa-download'></i> Export ke GPX</a>
               ...
               <!-- <a href="<?= site_url("sid_core/ajax_wilayah_dusun_google_maps/$data[id]")?>" class="btn btn-social btn-box btn-primary btn-sm pull-right" title="Ubah Wilayah"><i class='fa fa-map'></i> Ubah Wilayah</a>&nbsp;--> </div>
            </div>
          </div>
        </form>
      </div>
...
<script>
	...
	window.onload = function()
	{
		...
		<?php if (!empty($dusun_gis)): ?>
			set_marker(marker_dusun, '<?=addslashes(json_encode($dusun_gis))?>', '<?=ucwords($this->setting->sebutan_dusun)?>', 'dusun', "<?= favico_kabupaten()?>");
		<?php endif; ?>

		//OVERLAY WILAYAH RW
		<?php if (!empty($rw_gis)): ?>
			set_marker(marker_rw, '<?=addslashes(json_encode($rw_gis))?>', 'RW', 'rw', "<?= favico_kabupaten()?>");
		<?php endif; ?>

		//OVERLAY WILAYAH RT
		<?php if (!empty($rt_gis)): ?>
			set_marker(marker_rt, '<?=addslashes(json_encode($rt_gis))?>', 'RT', 'rt', "<?= favico_kabupaten()?>");
		<?php endif; ?>
...
/*		var legenda_desa = L.control({position: 'bottomright'});
		...
		peta_wilayah.on('overlayadd', function (eventLayer) {
		  if (eventLayer.name === 'Peta Wilayah Desa') {
		    setlegendPetaDesa(legenda_desa, peta_wilayah, <?=json_encode($kabupaten)?>, '<?=ucwords($this->setting->sebutan_desa)?>', '<?=$kabupaten['nama_desa']?>');
		  }
