...
class Track_model extends CI_Model {
  ...
  public function track_kabupaten($dari)
  {
  ...
  public function kirim_data()
  {
    ...
    $kabupaten = array(
     ...
     "email_kabupaten" => $config['email_kabupaten'],
    ...
    if ($this->abaikan($kabupaten)) return;

    $trackSID_output = httpPost($tracker."/index.php/api/track/kabupaten?token=".$this->token_sidega(), $kabupaten);
  ...
  // token_sidega digunakan sebagai tanda pemanggilan memang di lakukan dari aplikasi SIDeGa
  // Buat token_sidega kalau belum ada, menggunakan hash file LISENSI
  private function token_sidega()
  {
    if (empty($this->setting->token_sidega))
    {
      $lisensi = fopen('LICENSE', 'r');
      $token_sidega = sha1(file_get_contents($lisensi));
      ...
      $this->settingmodel->update_setting(['token_sidega' => $token_sidega]);
    }
    return $this->setting->token_sidega;
  }
  ...
  /*
    Jangan rekam, jika:
    - ada kolom nama wilayah kurang dari 4 karakter, kecuali kabupaten boleh 3 karakter
  ...
  */
  public function abaikan($data)
  {
    ...
    $abaikan = false;
    $kabupaten = trim($data['nama_kabupaten']);
    $kec = trim($data['nama_kecamatan']);
    $kab = trim($data['nama_kabupaten']);
    $prov = trim($data['nama_provinsi']);
    if ( strlen($kabupaten)<3 OR strlen($kec)<4 OR strlen($kab)<4 OR strlen($prov)<4 )
    {
      $abaikan = true;
    }
    elseif (preg_match($regex, $kabupaten) OR
