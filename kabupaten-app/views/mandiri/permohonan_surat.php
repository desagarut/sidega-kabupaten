  ...
  <div class="row">
    <div class="col-sm-3">
      <select class="form-control input-sm form-box" name="filter" onchange="formAction('mainform', '<?=site_url("{$this->controller}/filter")?>')">
        ...
        <option value="<?= $id?>" <?php if ($filter != '' and $filter == $id): ?>selected<?php endif ?>>
        <?= $nama?>
        </option>
        <?php endforeach; ?>
      </select>
          ...
          <table class="table table-bordered table-striped dataTable table-hover">
            <thead class="bg-purple">
              <tr>
                ...
                <th class="text-center">No HP Aktif</th>
                <th class="text-center">Jenis Surat</th>
                <!--<th>Status</th>-->
                <th class="text-center"><?= url_order($o, "{$this->controller}/{$func}/$p", 1, 'Tanggal Kirim'); ?></th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><?=$data['nama']?></td>
                <td class="text-center"><?=$data['no_hp_aktif']?></td>
                <td><?=$data['jenis_surat']?></td>
                <!--<td><?=$data['status']?></td>-->
                <td class="text-center"><?=tgl_indo2($data['created_at'])?></td>
                <td nowrap align="center"><a href="#" class="btn bg-warning disabled btn-box btn-sm" >
                  <?=$data['status']?>
                  </a></br>
    ...
    <div class="col-sm-6">
      <div class="dataTables_paginate paging_simple_numbers">
        <ul class="pagination">
          ...
          <li <?=jecho($p, $i, "class='active'")?>><a href="<?= site_url("{$this->controller}/index/$i/$o")?>">
            <?= $i?>
            </a></li>
          <?php endfor; ?>
          <?php if ($paging->next): ?>
