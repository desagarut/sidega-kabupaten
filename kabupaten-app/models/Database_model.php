...
class Database_model extends CI_Model {
	...
	private function reset_setting_aplikasi()
	{
		...
		$query = "
			...
			(5, 'sebutan_desa','kabupaten','Pengganti sebutan wilayah desa','',''),
	...
	public function migrasi_db_cri()
	{
		...
		$this->folder_kabupaten_model->amankan_folder_desa();
  ...
  private function migrasi_1806_ke_1807()
  {
	...
	private function migrasi_25_ke_26()
	{
		...
		$query = "
			...
			ON DUPLICATE KEY UPDATE
				...
				url = VALUES(url)";
		$this->db->query($query);
		// Tambah folder kabupaten/upload/media
		if (!file_exists('/kabupaten/upload/media'))
		{
			mkdir('kabupaten/upload/media');
			xcopy('kabupaten-contoh/upload/media', 'kabupaten/upload/media');
		}
		if (!file_exists('/kabupaten/upload/thumbs'))
		{
			mkdir('kabupaten/upload/thumbs');
			xcopy('kabupaten-contoh/upload/thumbs', 'kabupaten/upload/thumbs');
		}
	...
	{
		// Buat folder kabupaten/upload/pengesahan apabila belum ada
