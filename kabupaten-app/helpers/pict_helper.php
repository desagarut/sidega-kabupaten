...
// toko_produk
define ('MIME_TYPE_TOKO_PRODUK', serialize (array(
	'image/jpeg', 'image/pjpeg',
	'image/png',  'image/x-png' )));

define ('EXT_TOKO_PRODUK', serialize(array(
	".jpg", ".jpeg", ".png", "application/x-download",	"application/pdf",
)));
...
function UploadGallery($fupload_name, $old_foto='', $tipe_file='')
{
	...
	$dimensi = array("width"=>1280, "height"=>720);
...
* $dimensi = array("width"=>lebar, "height"=>tinggi)
* $lokasi contoh LOKASI_LOGO_KABUPATEN
...
function qrcode_generate($pathqr, $namaqr, $isiqr, $logoqr, $sizeqr, $backqr, $foreqr)
{
	...
	$params['savename'] = FCPATH.$config['imagedir'].$image_name; //simpan image QR CODE ke folder /kabupaten/upload/media/
	if (!empty($foreqr1))
	{
...
// Helper Toko Warga
function AmbilTokoProduk($toko_produk)
{
	$file_rumah = base_url() . LOKASI_TOKO_PRODUK . $toko_produk;
	return $file_toko_produk;
}

// Upload umum. Parameter lokasi dan file di $_FILES
function UploadKeLokasiTokoProduk($lokasi, $file, $fupload_name, $old_toko_produk="")
{
	$vfile_upload = $lokasi . $fupload_name;
	move_uploaded_file($file, $vfile_upload);
	unlink($lokasi . $old_toko_produk);
}

function UploadTokoProduk($fupload_name, $old_toko_produk="")
{
	$vfile_upload = LOKASI_TOKO-PRODUK . $fupload_name;
	move_uploaded_file($_FILES["satuan"]["tmp_name"], $vfile_upload);
	unlink(LOKASI_TOKO_PRODUK . $old_toko_produk);
}

function UploadTokoProduk2($fupload_name)
{
	$vdir_upload = LOKASI_TOKO_PRODUK;
	$vfile_upload = $vdir_upload . $fupload_name;
	move_uploaded_file($_FILES["toko_produk"]["tmp_name"], $vfile_upload);
