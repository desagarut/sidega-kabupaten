...
class Statistik extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->_list_session = ['lap', 'order_by', 'kecamatan', 'desa', 'dusun', 'rw', 'rt'];
		$this->modul_ini = 2;
	...
	*/
	public function daftar($aksi = '', $lap = '')
	{
		...
		$data['aksi'] = $aksi;
		$data['stat'] = $this->laporan_penduduk_model->judul_statistik($lap);
		$data['config'] = $this->header['kabupaten'];
	...
	private function get_cluster_session()
	{
		foreach ($this->_list_session as $list)
		{
			if (in_array($list, ['kecamatan', 'desa', 'dusun', 'rw', 'rt']))
	...
	public function chart_gis_desa($lap = 0, $kabupaten = '' )
	{
		($kabupaten) ? $this->session->set_userdata('kabupaten', ununderscore($kabupaten)) : $this->session->unset_userdata('kabupaten');
